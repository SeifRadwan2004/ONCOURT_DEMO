      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative flex-1 min-w-[200px] max-w-sm">
            ...
            <input type="text" placeholder="Search by name, entity, email or sport…"
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {(["All", ...PLANS] as const).map((p) => (
              <button key={p} onClick={() => setPlanFilter(p as Plan | "All")}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${planFilter === p ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/70"}`}>
                {p}
              </button>
            ))}
          </div>
        ...
        <div className="rounded-xl border border-border overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={COL_DEFS.length} className="px-4 py-10 text-center text-muted-foreground text-sm">No entities match your search or filters.</td></tr>
                ) : (
                  filtered.map((e, i) => (
                    <tr key={e.id} className={`border-b border-border last:border-0 hover:bg-muted/30 transition ${i % 2 === 0 ? "" : "bg-muted/10"}`}>
                      <td className="px-4 py-3 font-medium text-foreground whitespace-nowrap">{e.repName}</td>
                      <td className="px-4 py-3 text-muted-foreground">{e.email}</td>
                      <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{e.contact}</td>
                      <td className="px-4 py-3 text-muted-foreground">{e.sport}</td>
                      <td className="px-4 py-3 font-medium text-foreground whitespace-nowrap">{e.entityName}</td>
                      <td className="px-4 py-3 text-center text-foreground">{e.numAthletes}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold ${PLAN_BADGE[e.plan]}`}>{e.plan}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold ${STATUS_BADGE[e.status]}`}>{e.status}</span>
                      </td>
                      <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{fmtDate(e.renewalDate)}</td>
                    </tr>
                  ))
                )}
              </tbody>
          ...
          <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              <Field label="Rep Name" error={errors.repName}>
                <input type="text" placeholder="e.g. James Hartley" value={form.repName}
                  onChange={(ev) => setField("repName", ev.target.value)} className={inputCls(!!errors.repName)} />
              </Field>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field label="Subscription Plan" error={errors.plan}>
                  <select value={form.plan} onChange={(ev) => setField("plan", ev.target.value)} className={inputCls(!!errors.plan)}>
              ...
              <Field label="Number of Athletes" error={errors.numAthletes}>
                <input type="number" min={1} placeholder="e.g. 40" value={form.numAthletes}
                  onChange={(ev) => setField("numAthletes", ev.target.value)} className={inputCls(!!errors.numAthletes)} />
              </Field>
              <div className="grid grid-cols-2 gap-4">
              ...
              <div className="pt-2 flex items-center justify-end gap-3">
                ...
                <button type="submit"
                  className="px-5 py-2 rounded-lg text-sm font-medium bg-primary text-primary-foreground hover:opacity-90 transition">Add Entity</button>
              </div>
            </form>
          </div>
        </div>
